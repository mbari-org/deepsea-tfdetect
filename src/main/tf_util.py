...
Tensorflow utilities for logging trained models in mlflow
...
import shutil
import objdetpyfunc as objpyfunc

slim = tf.contrib.slim

def export_model(checkpoint, out_dir, pipeline_config_out):
    '''
    Helper class to export tensorflow graph for inference
    :param checkpoint:  path to checkpoint file
    :param out_dir:  directory to save exported graph
    :param pipeline_config_out: pipeline.config file
    :return:
    '''
    trained_checkpoint_prefix = checkpoint.split('.index')[0]
    saved_model_dir = os.path.join(out_dir, 'saved_model')

    pipeline_config = pipeline_pb2.TrainEvalPipelineConfig()
    with tf.gfile.GFile(pipeline_config_out, 'r') as f:
    ...
    # export in encoded_image_string_tensor so can serve out via REST
    tags, key = export_inference_graph('encoded_image_string_tensor', \
                                         pipeline_config, \
...
def log_model(checkpoint, pipeline_config_out, labels, image_dims, image_mean, artifact_path='model'):
    '''
    Creates a custom Pyfunc object detection  model for running inference

    :param checkpoint: checkpoint of model to export
    :param pipeline_config_out: path to pipeline.config of model to export
    ...
    '''
    with TempDir() as tmp:
        data_path = tmp.path('detect_model')
        os.mkdir(data_path)
        tags, keys = export_model(checkpoint, data_path, pipeline_config_out)
        objpyfunc.log_model(tmp, data_path, tags, keys, labels, image_dims, image_mean, artifact_path)
...
def write_saved_model(saved_model_path,
  ...
  with tf.Graph().as_default():
    with tf.Session() as sess:
      ...
      builder.save()
      return tf.saved_model.tag_constants.SERVING, signature_def
...
def export_inference_graph(input_type,
  ...
  if pipeline_config.HasField('graph_rewriter'):
    ...
    graph_rewriter_fn = graph_rewriter_builder.build(graph_rewriter_config,
                                                     is_training=False)
  tags, key = _export_inference_graph(
      input_type,
      detection_model,
...
def _export_inference_graph(input_type,
    ...
    tags, key = write_saved_model(saved_model_path, frozen_graph_def,
